...
import {
  GetMessage,
  GetMessageWithController,
  getRequest,
  postRequest,
} from "../utilz/Request/Request";
...
const Chat = () => {
  ...
  const [user, changeuser] = useState(userData);
  ...
  useEffect(() => {
    const getrequest = async () => {
      ...
      if (data.error) {
        ...
        let sortedChatrooms = [];
        if (data?.length > 0) {
          sortedChatrooms = data.sort((a, b) => {
            if (!a?.latestMessage?.sentAt) return 1;
            if (!b?.latestMessage?.sentAt) return -1;
            return (
              new Date(b.latestMessage.sentAt) -
              new Date(a.latestMessage.sentAt)
            );
          });
        }
        if (sortedChatrooms && sortedChatrooms[0])
          try {
            Changechatrooms((prev) => {
              if (!prev) return prev;
              return prev.map((chatroom) => {
                if (chatroom.roomId == sortedChatrooms[0].roomId) {
                  return {
                    ...chatroom,
                    seen: true,
                  };
            ...
            setCurrentRooms(sortedChatrooms[0]);
          } catch (err) {
  ...
  useEffect(() => {
    const GetMessageFunc = async () => {
      const response = await getRequest(
        `${process.env.REACT_APP_API_URL}/api/messages/gettop`
      );
      if (response.error) {
        setLoading(false);
      }
    ...
    if (chatrooms) {
      ...
      if (stompClient && chatrooms?.length > 0) {
        chatrooms.forEach((chatroom) => {
          ...
          return () => {
            const currentRoomIds = chatrooms?.map(
        ...
        if (NewMessage?.listMessage?.length === 0) {
          ...
          if (messagesEndRef.current) {
            messagesEndRef.current.scrollTop =
              NewMessage?.listMessage?.length * 10;
          }
        }
      }
      setLoading(false);
  ...
  return (
    <section
      className=" bg-bg1  p-2 text-white w-full"
        ...
        {currentRooms?.length !== 0 && (
          ...
          >
            {currentRooms?.length == 0 || changechatroomloading ? (
              ...
              <>
                <div className=" flex py-3 px-3 w-full justify-between items-center shadow-xl">
                ...
                >
                  {loading && (
                    <div className=" absolute top-0 right-1/2 animate-spin">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 24 24"
